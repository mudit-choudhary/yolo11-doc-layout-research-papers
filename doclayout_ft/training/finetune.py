...
Augmentation defaults reproduce the strongest model this project produced
(``yolo11s_doc_layout_imgsz_1024``, 0.769 mAP50-95). That run used Ultralytics'
own augmentation defaults, including ``fliplr=0.5`` and ``erasing=0.4``, with
``copy_paste`` and ``multi_scale`` off.

There is a tempting argument for setting ``fliplr=0.0`` on a document task: a
mirrored page never occurs at inference, and mirrored text destroys the
left-to-right structure that distinguishes a caption from a list item. The same
argument says ``erasing=0.4`` risks wiping the only instance of a rare class
off a page. Both are plausible, and **neither has been tested here in
isolation**. Every run that used them also enabled ``copy_paste`` and
``multi_scale``, and regressed. So they are exposed as flags, not baked in as
defaults, because defaulting to an untested setting would quietly stop this
script from reproducing the published models.

``copy_paste`` and ``multi_scale`` default off, which matches every run that
scored well. The one sweep that raised them regressed every model it touched by
0.016 to 0.060 mAP50-95, but it changed seven settings at once, so it does not
isolate a cause. See docs/EVALUATION.md for what that experiment can and cannot
support.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    aug.add_argument("--fliplr", type=float, default=0.5,
                     help="Horizontal flip probability (default: 0.5, matching "
                          "the best run). Setting 0.0 is well argued for "
                          "documents but untested here in isolation.")
    aug.add_argument("--copy-paste", type=float, default=0.0,
                     help="Copy-paste probability (default: 0.0, matching every "
                          "run that scored well). See the module docstring "
                          "before raising it.")
    aug.add_argument("--multi-scale", type=float, default=0.0,
                     help="Multi-scale training range as a fraction of imgsz "
                          "(default: 0.0, off, matching the best run)")
    aug.add_argument("--erasing", type=float, default=0.4,
                     help="Random erasing probability (default: 0.4, matching "
                          "the best run). Lowering it may help rare classes, "
                          "but that is untested here in isolation.")
    return parser
